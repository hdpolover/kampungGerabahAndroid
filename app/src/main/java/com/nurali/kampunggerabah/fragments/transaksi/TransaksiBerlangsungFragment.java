...
import com.nurali.kampunggerabah.adapters.TransaksiAdapter;
...
public class TransaksiBerlangsungFragment extends Fragment {
    ...
    public TransaksiBerlangsungFragment() {
        // Required empty public constructor
    }

    public static TransaksiBerlangsungFragment newInstance(String param1, String param2) {
        TransaksiBerlangsungFragment fragment = new TransaksiBerlangsungFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        view = inflater.inflate(R.layout.fragment_container_transaksi_berlangsung, container, false);
    ...
    private void tampilTrPengrajin(String id) {
        apiInterface.getTrBerlangsungPengrajin(id).enqueue(new Callback<TransaksiResponse>() {
            @Override
    ...
    private void tampilTr(String id) {
        apiInterface.getTrBerlangsung(id).enqueue(new Callback<TransaksiResponse>() {
            @Override
            public void onResponse(Call<TransaksiResponse> call, Response<TransaksiResponse> response) {
                if (response.body().status) {
                    List<TransaksiResponse.TransaksiModel> list = new ArrayList<>();
